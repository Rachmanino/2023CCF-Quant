...
class Tcn_Module(nn.Module):
    ...
    def forward(self, stock_info, embedding_intput):
        ...
        embedded_price = self.tcn(data_trans_no_index)[:, :, -1]
        ##embedded_price  shape [batch, tcn_embeddings]

        ##embedded_wind1  shape embedded_wind1
        embedded_wind1 = self.wind1_embedding_layer(embedding_intput[0])
        embedded_wind2 = self.wind2_embedding_layer(embedding_intput[1])
        embedded_company = self.company_embedding_layer(embedding_intput[2])

        #####temp   embedded_wind1
        temp = embedded_price + embedded_wind1 + embedded_wind2 + embedded_company




        out = self.mlp( self.drop(temp))
