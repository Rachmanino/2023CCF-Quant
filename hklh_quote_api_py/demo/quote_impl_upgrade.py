...
class QuoteImpl(QuoteApi):
    ...
    def onTick(self,tick):
        ...
        if self.last_prices[tick['hjcode']] == -1:
            self.last_prices[tick['hjcode']] = tick['vwap']
        # 2.判断是否需要交易
        self.cnt[tick['hjcode']] += 1
        if self.last[tick['hjcode']] == 'buy' and \
                ((preds_next_time - preds_next_period > THRESHOLDS and data[-2,0] * (1.0 + preds_next_time/100) > self.last_prices[tick['hjcode']])
            ...
            self.last[tick['hjcode']] = 'sell'
                ...
                ((preds_next_period - preds_next_time > THRESHOLDS and data[-2,0] * (1.0 + preds_next_time/100) < self.last_prices[tick['hjcode']])
                 or self.cnt[tick['hjcode']] >= MAXTIME):
            self.sell(tick['hjcode'])
            self.cnt[tick['hjcode']] = 0
            self.last_prices[tick['hjcode']] = -1 #下一分钟才知道价格
