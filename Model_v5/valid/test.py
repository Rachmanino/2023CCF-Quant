...
for i in tqdm(range(len(DataSets))):
    ...
    for data, embbeding_input, target in dataset:
        ...
        cnt = 0
        with torch.no_grad():
            # print(data[:-1].unsqueeze(0).shape, embbeding_input.shape)
            # pause()
            if embbeding_input[0].numel() == 1:
                for i in range(len(embbeding_input)):
                    embbeding_input[i] = embbeding_input[i].unsqueeze(0)
        ...
        if last == 'buy' and \
            ...
            last = 'sell'
            print(f' sell, pnl = ({last_prices} - {data[-1, 4]})/{last_prices} = {pnl*1e4}')
        elif last != 'buy' and \
            ...
            if last == None:   # buy
                ...
                last_prices = data[-1, 4]
            last = 'buy'
            print(f' buy, pnl = ({last_prices} - {data[-1, 4]})/{last_prices} = {pnl*1e4}')
        ###################### END STRATEGY #######################
