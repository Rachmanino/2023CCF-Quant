...
class Stock_Dataset(Dataset):
    def __init__(self, data_dir,name, predict_period=10,day_windows=30, flag='train', device ="cpu", scale_money =SCALE_MONEY, scale_volume= SCALE_VOLUME) -> None:
        ...
        self.df_categories = pd.read_csv('../dataset/categories.csv')
    ...
    def __getitem__(self, index:int):
        ...
        reward_ratio = (next_data_close - data_vwap) / data_vwap
        next_period = (next_period - data_vwap) / data_vwap

        ####所有的价格都除以最后一天的收盘价,可以把数据价格统一归一化到1附近
        data[:,:5] = data[:,:5] / data_vwap.reshape((1,-1,1))     ####np.repeat(last_close, self.indicator_num, axis=2)
        ...
        return data, embedding_intput, target
